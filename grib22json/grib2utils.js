...
const parseData = function (decodedGrib) {
    ...
    if (template == '5.0' || template == '5.1' || template == '5.2' || template == '5.3' || template == '5.40') {
        let compression = {};
        ...
        if (template == '5.40') {
            compression.bitsPerValue = getContentByInfo(decodedGrib[5], 'Number of bits required to hold the resulting scaled and referenced data values. (i.e. The depth of the grayscale image.) (see Note 2)');
        } else {
        ...
        compression.compress = (value => {
            const c1 = Math.pow(2, compression.binaryScaleFactor)
            const c2 = Math.pow(10, compression.decimalScaleFactor)
            return   ((value * c2) - compression.refValue) / c1;
        });
    ...
    // Template 40 : Grid point data - JPEG 2000 code stream format
    // https://codes.ecmwf.int/grib/format/grib2/templates/5/40/
    // https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_temp5-40.shtml
    else if (template == '5.40') {
        // console.log("Template 5.40");
        let compression = data.compression;
        const debug = false;
        if (debug) {
            const paramName = product["Parameter number (see Code table 4.2)"].abbreviation;
            const path = `/home/michael/tmp/data_${paramName}.jp2`;
            fs.appendFileSync(path, new Uint8Array(rawData));
        }

        let t0 = new Date();
        
        let items = decodeJPXImage(rawData);
        
        for (let i = 0; i < items.length; i++) {
            values[i] = compression.decompress(items[i]);
        }

        // Performance measuring
        let elapsed = new Date() - t0;
        // console.log('---------------------------------------------------------');
        console.warn('Decoded image in %dms', elapsed);
        // console.log('---------------------------------------------------------');
